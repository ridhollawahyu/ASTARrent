...
$where = "WHERE m.kodeProdi_mahasiswa = '$dept' AND (tp.idAset IS NOT NULL OR f.tipeFasilitas = 'Akademik')";
...
$sql = "SELECT tp.*, tpm.tanggalPengembalian, m.namaMahasiswa, m.nimMahasiswa, a.namaAset, f.namaFasilitas,
...
while ($row = mysqli_fetch_assoc($q)) {
    $data_report[] = $row;
    $nm_brg = $row['namaAset'] ?: $row['namaFasilitas'];
    $barang_count[$nm_brg] = ($barang_count[$nm_brg] ?? 0) + 1;
...
arsort($barang_count);
$top_barang = key($barang_count) ?? 'Tidak Ada';
...
<div class="print-header">
    <h3>LAPORAN SIRKULASI AKADEMIK</h3>
    <div class="print-date">Dicetak: <?= date('d-m-Y H:i') ?> | Oleh: Tenaga Pendidik (<?= $dept ?>)</div>
</div>
...
<div class="card shadow-sm border-0 no-print mb-4" style="border-radius: 15px;">
    <div class="card-header bg-astar text-white" style="border-radius: 15px 15px 0 0;">
        <h5 class="mb-0 fw-bold">Filter Laporan Sirkulasi</h5>
    </div>
    <div class="card-body p-4 bg-light">
        <form method="GET" action="" class="row g-3">
            ...
            <div class="col-12 text-end mt-4"><button type="submit" class="btn btn-astar fw-bold">Filter</button><button type="button" onclick="window.print()" class="btn btn-danger fw-bold ms-2">Cetak PDF</button><button type="button" onclick="exportToCSV('Laporan_Sirkulasi')" class="btn btn-success fw-bold ms-2">Excel</button></div>
        </form>
    </div>
</div>
...
<div class="row g-4 mb-4">
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm p-3 h-100" style="border-left: 5px solid #dc3545 !important;">
            <p class="text-muted fw-semibold mb-1">Mhs Paling Sering Telat</p>
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm p-3 h-100" style="border-left: 5px solid #198754 !important;">
            <p class="text-muted fw-semibold mb-1">Barang Paling Laris</p>
            <h5 class="fw-bold text-success"><?= $top_barang ?></h5>
        </div>
    </div>
</div>
...
<div class="card shadow-sm border-0" style="border-radius: 15px;">
    <div class="card-body p-4 table-responsive">
        <table class="datatable-astar table table-hover border text-center align-middle" id="tableLaporan">
            ...
            <tbody>
                <?php $no = 1;
                foreach ($data_report as $row): $nm_brg = $row['namaAset'] ?: $row['namaFasilitas']; ?>
                    <tr>
                        <td><?= $no++ ?></td>
                        <td class="text-start"><b><?= $row['namaMahasiswa'] ?></b><br><small><?= $row['nimMahasiswa'] ?></small></td>
                        <td class="text-start fw-bold text-secondary"><?= $nm_brg ?></td>
