...
// HANYA MENGAMBIL YANG SUDAH SUKSES DICAIRKAN (BERARTI ASET BARU BERTUMBUH)
...
$sql = "SELECT tp.*, k.namaKategori, u.namaUser as namaTendik FROM transaksi_pengadaan tp JOIN kategori k ON tp.idKategori = k.idKategori LEFT JOIN users u ON tp.idTendik = u.idUser $query_where ORDER BY tp.tanggalPengadaan DESC";
$data_report = [];
...
$total_unit_baru = array_sum(array_column($data_report, 'jumlah'));
...
if (isset($_GET['export_pdf']) && $_GET['export_pdf'] == '1') {
    ...
    $html .= '<div class="kop">' . $img_tag . '<h3>LAPORAN PERTUMBUHAN ASET KAMPUS</h3><p>Periode Pengadaan Selesai: ' . (!empty($tgl_awal) ? date('d/m/Y', strtotime($tgl_awal)) : 'Awal') . ' s/d ' . (!empty($tgl_akhir) ? date('d/m/Y', strtotime($tgl_akhir)) : 'Akhir') . '</p><p>Dicetak Oleh: ' . htmlspecialchars($_SESSION['username'] ?? 'Kepala GA') . ' | Tanggal Cetak: ' . date('d/m/Y H:i:s') . '</p></div>';
    $html .= '<table><thead><tr><th width="5%">No</th><th width="15%">ID Pengadaan</th><th width="25%">Barang/Kategori</th><th width="20%">Pemohon</th><th width="15%">Total Unit Tiba</th><th width="20%">Tgl Diajukan</th></tr></thead><tbody>';
    $no = 1;
    foreach ($data_report as $row) {
        $html .= '<tr><td>' . $no++ . '</td><td>' . $row['idPengadaan'] . '</td><td style="text-align:left;"><b>' . $row['namaKebutuhan'] . '</b><br>' . $row['namaKategori'] . '</td><td>' . $row['namaTendik'] . '</td><td style="font-weight:bold;">' . $row['jumlah'] . ' Unit</td><td>' . date('d-m-Y', strtotime($row['tanggalPengadaan'])) . '</td></tr>';
    }
    ...
    $dompdf->render();
    $dompdf->stream("Laporan_PertumbuhanAset_KepalaGA.pdf", array("Attachment" => true));
    exit;
}
...
<ul class="nav nav-tabs mb-4 border-bottom-0 gap-1">
    <li class="nav-item"><a class="nav-link active fw-bold text-astar border border-bottom-0 px-4 py-2" href="laporan_pertumbuhan.php" style="border-radius: 8px 8px 0 0; background-color: #fff;">Pertumbuhan Aset (Sukses E-Proc)</a></li>
    <li class="nav-item"><a class="nav-link fw-bold text-secondary px-4 py-2 border border-bottom-0" href="laporan_reparasi.php" style="border-radius: 8px 8px 0 0; border-color: transparent;">Global Reparasi</a></li>
</ul>
...
<div class="card shadow-sm border-0 mb-4" style="border-radius: 15px;">
    <div class="card-header d-flex justify-content-between align-items-center" style="background-color: #1d4197; border-radius: 15px 15px 0 0;">
        <h5 class="mb-0 text-white fw-bold"><i class="bi bi-graph-up-arrow me-2"></i>Laporan Pertumbuhan Aset Kampus</h5>
        <a href="../../dashboards/kepalaga_home.php" class="btn btn-outline-light btn-sm fw-bold"><i class="bi bi-arrow-left"></i> Kembali ke Dashboard</a>
    </div>
    <div class="card-body p-4 bg-light" style="border-radius: 0 0 15px 15px;">
        <form method="GET" action="" class="row g-3">
            ...
            <div class="col-12 mt-3 text-end">
                <a href="laporan_pertumbuhan.php" class="btn btn-light border fw-bold text-secondary px-3 ms-2">Reset</a>
                <button type="submit" name="export_pdf" value="1" class="btn btn-danger fw-bold px-4 ms-2"><i class="bi bi-file-pdf-fill me-1"></i> Generate PDF</button>
            </div>
        </form>
    </div>
</div>
...
<div class="row g-4 mb-4">
    <div class="col-md-6">
        <div class="card border-0 shadow-sm p-3 h-100" style="border-radius: 12px; border-left: 5px solid #1d4197 !important;">
            <p class="text-muted mb-1 fw-semibold" style="font-size: 0.75rem;">TRANSAKSI PENGADAAN SUKSES</p>
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm p-3 h-100" style="border-radius: 12px; border-left: 5px solid #198754 !important;">
            <p class="text-muted mb-1 fw-semibold" style="font-size: 0.75rem;">TOTAL PERTUMBUHAN BARANG (UNIT)</p>
            <h4 class="fw-bold mb-0 text-success">+ <?= $total_unit_baru ?> Unit Baru</h4>
        </div>
    </div>
</div>
