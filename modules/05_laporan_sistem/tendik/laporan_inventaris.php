...
// LOGIKA UNION UNTUK MENGGABUNGKAN ASET DAN FASILITAS AKADEMIK
...
$sql = "SELECT * FROM (
            SELECT idAset AS id_brg, namaAset AS nama, 'Aset' AS tipe, kondisiAset AS kondisi, ketersediaanAset AS sedia 
            FROM aset 
            UNION ALL 
            SELECT idFasilitas AS id_brg, namaFasilitas AS nama, 'Fasilitas Akademik' AS tipe, kondisiFasilitas AS kondisi, ketersediaanFasilitas AS sedia 
            FROM fasilitas WHERE tipeFasilitas = 'Akademik'
...
if (isset($_GET['export_pdf']) && $_GET['export_pdf'] == '1') {
    ...
    $html .= '<div class="kop">' . $img_tag . '<h3>LAPORAN STATUS INVENTARIS AKADEMIK</h3><p>Dicetak Oleh: ' . htmlspecialchars($_SESSION['username'] ?? 'Tendik') . ' | Tanggal Cetak: ' . date('d/m/Y H:i:s') . '</p></div>';
    ...
    $dompdf->render();
    $dompdf->stream("Laporan_Inventaris_Tendik.pdf", array("Attachment" => true));
    exit;
}
...
<ul class="nav nav-tabs mb-4 border-bottom-0 gap-1">
    ...
    <li class="nav-item"><a class="nav-link fw-bold text-secondary px-4 py-2 border border-bottom-0" href="laporan_pengadaan.php" style="border-radius: 8px 8px 0 0; border-color: transparent;">Pengajuan Pengadaan</a></li>
...
<div class="card shadow-sm border-0 mb-4" style="border-radius: 15px;">
    <div class="card-header d-flex justify-content-between align-items-center" style="background-color: #1d4197; border-radius: 15px 15px 0 0;"">
        <h5 class=" mb-0 text-white fw-bold"><i class="bi bi-box-seam-fill me-2"></i>Laporan Status Inventaris Akademik</h5>
...
<div class="row g-4 mb-4">
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm p-3 h-100" style="border-radius: 12px; border-left: 5px solid #ffc107 !important;">
            <p class="text-muted mb-1 fw-semibold" style="font-size: 0.75rem;">SEDANG DIPINJAM</p>
            <h4 class="fw-bold mb-0 text-warning text-dark"><?= count(array_filter($data_report, fn($r) => $r['sedia'] === 'Dipinjam')) ?> Item</h4>
        </div>
    </div>
</div>
